...
def apply_chain_to_cfg(cfg: dict[str, Any], chain: dict[str, Any]) -> None:
    ...
    cpe = normalize_ip(str(chain.get("cpe_lan_ipv4", "")).split("/")[0])
    if bts:
        cfg["_preflight_bts_lan_ipv4"] = bts
    if cpe:
        cfg["_preflight_cpe_ipv4"] = cpe
...
def mark_suite_healthy(
    ...
    *,
    ...
    cpe = normalize_ip(str(cpe_ip).split("/")[0])
    if bts:
...
def chain_allows_preflight_skip(
    ...
    *,
    ...
    if not chain.get("bts_ping_ok") or not chain.get("local_pc_ok"):
        return False
    if require_cpe:
        if not chain.get("cpe_lan_ipv4") or not chain.get("cpe_ping_ok"):
            return False
